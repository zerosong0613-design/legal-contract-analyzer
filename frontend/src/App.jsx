...
export default function App() {
  const [tab, setTab] = useState("analyze"); // "analyze" | "dashboard"
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <header className="border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight">ContractLens</h1>
            <p className="text-xs text-slate-500">계약서 AI 분석 · 리드타임 등급 · 리스크 진단</p>
          </div>
        </div>
        <nav className="flex gap-1">
          {[["analyze", "📋 계약서 분석"], ["dashboard", `📂 대시보드 (${records.length})`]].map(
            ([t, label]) => (
              <button
                key={t}
                onClick={() => setTab(t)}
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {tab === "dashboard" && (
          <Dashboard records={records} onRemove={(id) => setRecords((p) => p.filter((r) => r.id !== id))} />
        )}
      </main>
    </div>
  );
