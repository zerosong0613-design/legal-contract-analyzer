    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-5">
      ...
      <div className="grid grid-cols-3 divide-x divide-slate-100 px-2 py-4">
        {[
          ...
          { step: "02", icon: "🤖", title: "AI 자동 분석", desc: "AI가 조항을 분석하고\n등급 산정 (10초)" },
...
export default function App() {
  ...
  const [pendingReceivedDate, setPendingReceivedDate] = useState("");
  ...
  const handleAnalyze = async ({ file, text, assignee = "", receivedDate = "" }) => {
    setLoading(true);
    setError(null);
    setResult(null);
    setPendingAssignee(assignee);
    setPendingReceivedDate(receivedDate || new Date().toISOString().slice(0, 10));
    try {
      let res;
      if (file) {
  ...
  const addToLog = (record) => {
    const newRecords = [
      ...
      {
        ...record,
        assignee: pendingAssignee || record.assignee || "",
        id: Date.now(),
        date: pendingReceivedDate || new Date().toISOString().slice(0, 10),
      },
    ];
    updateRecords(newRecords);
    setResult(null);
    setPendingAssignee("");
    setPendingReceivedDate("");
    setTab("dashboard");
  };
